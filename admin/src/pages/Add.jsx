...
const Add = ({ token }) => {
    ...
    const fetchOptions = async () => {
      try {
        ...
        const subCatRes = await axios.get(
          backendUrl + "/api/category/sub/list"
        );
        ...
        if (subCatRes.data.success) {
          ...
          if (subCatRes.data.subCategories.length > 0)
            setSubCategory(subCatRes.data.subCategories[0].name);
        }
      } catch (error) {
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      const formattedSizes = Object.entries(sizes).map(([size, quantity]) => ({
        size,
        quantity,
      }));
          ...
          {["S", "M", "L", "XL", "XXL"].map((size) => (
            <div key={size} className="flex items-center gap-4">
              <div
                ...
                } border px-4 py-1 cursor-pointer w-16 text-center`}
              >
                {size}
              </div>
