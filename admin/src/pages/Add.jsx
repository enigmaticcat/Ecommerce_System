...
const Add = ({token}) => {
    ...
    const fetchOptions = async () => {
      try {
        ...
        if(catRes.data.success) {
          ...
          // Set default if list not empty
          if(catRes.data.categories.length > 0) setCategory(catRes.data.categories[0].name);
        }

        if(subCatRes.data.success) {
          setSubCatList(subCatRes.data.subCategories);
           // Set default if list not empty
           if(subCatRes.data.subCategories.length > 0) setSubCategory(subCatRes.data.subCategories[0].name);
        }
      } catch (error) {
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      // Convert sizes object {S: 10, M: 5} to stringified array [{size: 'S', quantity: 10}, ...]
      const formattedSizes = Object.entries(sizes).map(([size, quantity]) => ({ size, quantity }));
